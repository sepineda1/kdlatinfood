...
class Dash extends Component
{
    ...
    public function render()
    {
        ...
        // Contar los envíos con estado PENDIENTE
        $pendingCount = Sale::where('status_envio', 'PENDIENTE')->count();
        $totalProd = Product::all()->count();
        $totalProdCr = Product::where('estado', 'CRUDO')->count();
        $totalProdCc = Product::where('estado', 'PRECOCIDO')->count();
        $ultimoSaborCreado = Product::latest('created_at')->first();
        // Contar los envíos con estado ACTUAL
        $currentCount = Sale::where('status_envio', 'ACTUAL')->count();
        $productoMasVendido = Product::select(
            'products.id',
            'products.name',
        ...
        if ($productoMasVendido) {
            $topSellingSaborEQ = Product::find($productoMasVendido->id);
            $nombreSabor = $topSellingSaborEQ->name;
        } else {
            $nombreSabor = 'Ningún sabor';
        }
      //  dd($nombreSabor);
        // Contar los envíos con estado FIN
        $finishedCount = Sale::where('status_envio', 'FIN')->count();
        $this->getWeekSales();
        $this->getTop5();
        $this->getSalesMonth();

        return view('livewire.dash.component', compact(
            'pendingCount',
            'currentCount',
            'finishedCount',
            'totalProd',
    ...
    {
        $this->top5Data = SaleDetail::join('products as p', 'sale_details.presentaciones_id', 'p.id')
            ->select(
