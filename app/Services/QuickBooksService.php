...
class QuickBooksService
{
    ...
    /*private function updateAccessToken()
    {
        ...
        try {
            ...
            if (!$quickbook_credentials) {
                throw new Exception("No hay credenciales activas en la base de datos");
            }

            $access_token = $quickbook_credentials->access_token; // ACADASDASDASD
            $refresh_token = $quickbook_credentials->refresh_token; //


            $this->setDataService($access_token, $refresh_token);
            $OAuth2LoginHelper = $this->dataService->getOAuth2LoginHelper();
            $RR = $OAuth2LoginHelper->getAccessToken();
            //dd($RR['refresh_token']);
            ...
            // Guardar los nuevos tokens en la BD y en caché
            $quickbook_credentials->access_token = $accessTokenValue;
            $quickbook_credentials->refresh_token = $refreshTokenValue;
            $quickbook_credentials->save();
    ...
    {   
        ...
        try {
            ...
            $incomeAcctId = $this->findAccountIdByName('Sales of Product');     // p.ej. “Sales of Product Income”
            //$assetAcctId  = $this->findAccountIdByName('Inventory Asset');      // “Inventory Asset”
            $cogsAcctId   = $this->findAccountIdByName('Cost of Goods Sold');    // “Cost of Goods Sold”
            ...
            //$accounts = $this->dataService->Query("SELECT * FROM Account WHERE AccountType = 'Income'");
            //Log::error("Código de error: " . json_encode($accounts));
            ...
            if (!$createdProduct) {
                ...
                if ($error) {
                    Log::error("Error al agregar producto: " . $error->getResponseBody());
                    Log::error("Código de error: " . $error->getHttpStatusCode());
                }
                return false;
            }
    ...
    public function create_product($product)
    {
        ...
        try {
            ...
            // 5) Crea el Item NonInventory en QB
            /*$qb_product = ItemQB::create([
                "Name"             => $name_product,
                "UnitPrice"        => $product->price,
                "Description"      => $product->product->descripcion,
                ...
                // usa "Value" con V mayúscula y nunca pases null
                "IncomeAccountRef" => ["Value" => $incomeAcctId],
                "ExpenseAccountRef"=> ["Value" => $cogsAcctId],
    ...
    public function create_client($client)
{
    ...
    /*public function create_client($client)
    {
        try {
            ...
            $customer = $this->dataService->Add($qb_customer);
    ...
    public function create_client_sync(array $client)
    {
        ...
        try {
            ...
            // Agregar cliente a QuickBooks
            $customer = $this->dataService->Add($qb_customer);
            //falta acutlizar el QB del cliente
            return true;

        } catch (ServiceException $e) {
            // Registrar el error para depuración
            //\Log::error('Error al crear cliente en QuickBooks: ' . $e->getMessage());
            return false;
        }
    }
    ...
    {
        if (self::$disabled) return true;
          //Toca enviar toda la factura
          $sale = Sale::findOrFail($sale_id);
          $saleDetails = SaleDetail::where('sale_id', $sale->id)->get();


          $cliente = Customer::find($sale->CustomerID);
          ...
          foreach ($saleDetails as $producto) {
              $QB_Id = 34;
              $item = Presentacion::find($producto->presentaciones_id);
              if ($item && $item->QB_id === null) {
                  if ($this->create_product($item)) {
                      $item = Presentacion::find($producto->presentaciones_id);
                      $QB_Id = $item->QB_id;
                  }
              }
              //Agrego el producto como detalle de venta.
              $lineasFactura[] = [
                  "DetailType" => "SalesItemLineDetail",
                  "Amount" => $producto->price * $producto->quantity,
                  "SalesItemLineDetail" => [
                      "ItemRef" => [
                          "value" => $QB_Id
                      ],
                      "Qty" => $producto->quantity
                  ]
              ];
              $totalAmount += $producto->price * $producto->quantity;
              //Buscar si un producto tiene descuento
              $discount = Discounts::where('customer_id', $sale->CustomerID)->where('presentacion_id', $producto->presentaciones_id)->first();
              if ($discount) {
                  $descuento += ($producto->price * $producto->quantity) * ($discount->discount / 100);
          ...
          if ($descuento > 0) {
              $lineasFactura[] = [
                  ...
                  "Amount" => $descuento, // El descuento es negativo
                  "DiscountLineDetail" => [
                      "PercentBased" => false, // O true si prefieres porcentaje
                      "DiscountAccountRef" => [
                          "value" => "26" // Debes colocar el ID de la cuenta de descuentos ANTES 26
                      ]
                  ]
              ];
          }
          if (!$cliente->QB_id) {
              if ($this->create_client($cliente)) {
                  $cliente = Customer::find($sale->CustomerID);
              }
          }
          ...
          $qb_invoice = Invoice::create([
            ...
            "Line" => $lineasFactura,
            "CustomerRef" => ["value" => $cliente->QB_id],
            "DocNumber" => $sale_id
        ]);

          try {
            $factura = $this->dataService->Add($qb_invoice);
    
            // si no vino objeto, hubo error  
            if (! $factura) {
                $error = $this->dataService->getLastError();
            ...
            \Log::error("QuickBooks Exception al crear invoice: " . $e->getMessage());
            return false;
        }
          ...
          /*$existing = $this->dataService->Query("select * from Invoice where DocNumber = '".$sale_id."'");
          if (!empty($existing)) {
              Log::info("Factura ya existe con DocNumber 961, ID: " . $existing[0]->Id);
              return false; // o manejar como actualización si lo deseas
          }
...
public function updateInvoice(int $sale_id): bool
{
    ...
    /*public function update_invoice($sale_id)
    {   
        if (self::$disabled) return ['success' => 'Factura mockeada'];
        try{
            $sale = Sale::findOrFail($sale_id);
            $saleDetails = SaleDetail::where('sale_id', $sale->id)->get();
            $cliente = Customer::find($sale->CustomerID);
            ...
            foreach ($saleDetails as $producto) {
                ...
                $discount = Discounts::where('customer_id', $sale->CustomerID)
                    ->where('presentacion_id', $producto->presentaciones_id)
                    ->first();
                if ($discount) {
                    $descuento += ($producto->price * $producto->quantity) * ($discount->discount / 100);
                }
            ...
            if ($descuento > 0) {
                ...
                $lineasFactura[] = [
                    ...
                    "DiscountLineDetail" => [
                        "PercentBased" => false,
                        "DiscountAccountRef" => [
                            "value" => "26" // Ajusta según tu cuenta de descuentos
                        ]
                    ]
                ];
            }

            // Asegurarse de que el cliente tenga QB_id
            if (!$cliente->QB_id) {
                if ($this->create_client($cliente)) {
                    $cliente = Customer::find($sale->CustomerID);
                }
            ...
            // 🔄 BUSCAR FACTURA EXISTENTE EN QUICKBOOKS
            $existingInvoice = $this->dataService->Query("SELECT * FROM Invoice WHERE DocNumber = '$sale_id'");

            if (count($existingInvoice) > 0) {
                $invoice = $existingInvoice[0];
                $invoice->Line = $lineasFactura;
                $invoice->CustomerRef = ["value" => $cliente->QB_id];
                $invoice->TotalAmt = $totalAmount;

                try {
                    $updatedInvoice = $this->dataService->Update($invoice);
                    $this->adjustments_inventory($saleDetails);
                    return ['success' => 'Factura actualizada en QuickBooks.', 'factura_id' => $updatedInvoice->Id];
                } catch (Exception $e) {
                    \Log::error("Error al actualizar factura: " . $e->getMessage());
                    return ['error' => 'No se pudo actualizar la factura en QuickBooks.'];
                }
            } else {
                return ['error' => 'No se encontró la factura en QuickBooks para actualizar.'];
            }
