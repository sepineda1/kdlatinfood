    ...
    <div class="col-sm-12">
        <div class="widget widget-chart-one">
            ...
            <div class="widget-content">
                                    ...
                                    <table class="table table-bordered table-striped mt-1">
                                        <thead class="text-white" style="background: #ff5100">
                                            <tr>
                                                ...
                                                <th class="table-th text-center text-white d-none">Status</th>
                                                <th class="table-th text-center text-white d-none">QR</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @php
                                                        ...
                                                        <tr>
                                                            @foreach ($cliente as $clientes)
                                                                @if ($pendientes->sales->CustomerID == $clientes->id)
                                                                    <th class="table-th text-center d-flex"><button
                                                                            class="btn btn-warning btn-rounded mr-1"
                                                                            onclick="getSaleDetails({{ $pendientes->sales->id }})"><i
                                                                                class="fas fa-list"></i></button>
                                                                                <button
                                                                                class="btn btn-rounded btn-primary"
                                                                                onclick="loader();Livewire.emit('openEditModal', {{ $pendientes->sales->id }})"
                                                                            >
                                                                                <i class="fas fa-edit"></i> 
                                                                            </button>
                                                                            </th>
                                                                    <th class="table-th text-center">
                                                                        {{ $pendientes->sales->id }}</th>
                                                                    <th class="table-th text-center">
                                                                        {{ $pendientes->sales->fecha_carga }}</th>
                                                                    <th class="table-th text-center">
                                                                        {{ $clientes->name }}
                                                            ...
                                                            @foreach ($operario as $operarios)
                                                                @if ($pendientes->id_transport == $operarios->id)
                                                                    ...
                                                                    <th class="table-th text-center">
                                                                        {{ $operarios->compañia }}</th>
                                                                @endif
                                                            @endforeach

                                                            <th class="table-th text-center d-none">
                                                                <button class="btn btn-success btn-rounded btnQR"
                                                            ...
                                                            <th class="table-th text-center d-none">
                                                                <button class="btn btn-secondary btn-rounded btnQR">
                ...
                @include('livewire.envios.modal-detalles-orden')
                @include('livewire.envios.modal-edit-orden')
                ...
                <script type="text/javascript">
                    ...
                    // Delegación para botones .btnQR
                    /*document.body.addEventListener('click', function (e) {
                        if (e.target.classList.contains('btnQR')) {
                            var saleId = e.target.getAttribute('data-sale-id');
                            ventaId = saleId;

                            //$('#modalSaleDetails').modal('hide');
                            $('#modalScan').modal('show');
                        ...
                        // Delegación para botones que abren detalles (ícono lista)
                        if (e.target.classList.contains('btnDetalles')) {
                            const saleId = e.target.getAttribute('data-sale-id');
                            getSaleDetails(saleId); // Aquí llamamos al detalle
                        }
                    })*/
                    ;

                    function setTodalScan(ventaId) {
                        //var saleId = e.target.getAttribute('data-sale-id');
                        ventaId = ventaId;
                        document.getElementById('orderSelect').value = ventaId;
                        ...
                        /*document.getElementById('CargarQR').addEventListener('click', function () {
                            fetch('/intranet/public/qr/' + ventaId, {
                                method: 'POST',
                                headers: {
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                    'Content-Type': 'application/json'
                    ...
                    $(document).on('show.bs.modal', '.modal', function() {
                        ...
                        const $backdrop = $('.modal-backdrop').not('.stacked').last();
                        $backdrop.css('z-index', modalZIndex - 1).addClass('stacked');

                        // Este fix forza el focus al modal activo
                        setTimeout(() => {
                            $(this).focus();
                        }, 200);
                    });
                    ...
                    function getSaleDetails(saleID, isEdit = false) {
                        ...
                        fetch('sale/details/' + saleID, {
                            ...
                            .then(response => response.json())
                            .then(data => {
                                document.getElementById('orderNumber').innerText = saleID;
                                ...
                                data.saleDetails.forEach(item => {
                                    ...
                                    var button =
                                        `<button class="btn btn-primary btn-rounded"><i class="fas fa-check"></i></button>`;
                                    if (item.scanned == 0) {
                                        button = `
                                <button class="btn btn-success btn-rounded btnQR"
                                        data-sale-presentacion="${item.presentacion_id}"
                                        onclick="setTodalScan(${saleID})"
                                        title="Update On transit">
                                ...
                                <td class="text-center"><span class="badge badge-success"><i class="fas fa-barcode" style="font-size:35px;"></i> ${item.codigoBarras != null ? item.codigoBarras : ""}</span></td>
                            </tr>`;
                                ...
                                if (isEdit) {
                                    var btnHtml =
                                        `<button class="btn btn-primary btn-rounded" onclick="editarOrden(${saleID})" ><i class="fas fa-pencil-alt"></i> Editar Orden</button>`;
                                    document.getElementById('displayButton').innerHTML = btnHtml;
                                }

                                // Mostrar los datos y ocultar spinner
                                                    ...
                                                    @foreach ($operario as $operarios)
                                                        @if ($pendientes->id_transport == $operarios->id)
                                                            @foreach ($cliente as $clientes)
                                                                @if ($pendientes->sales->CustomerID == $clientes->id)
                                                                    <tr>
                                                                        <td class="table-th text-center d-flex"><button class="btn btn-warning btn-rounded"
                                                                                onclick="getSaleDetails({{ $pendientes->sales->id }},true)"><i
                                                                                    class="fas fa-list"></i></button>
                                                                              <button
                                                                                class="btn btn-rounded btn-primary"
                                                                                onclick="loader();Livewire.emit('openEditModal', {{ $pendientes->sales->id }})"
                                                                        ...
                                                                        <th class="table-th text-center">
                                                                            {{ $pendientes->sales->id }}</th>
                                                                        <th class="table-th text-center">
                                                                            {{ $pendientes->sales->fecha_escaneo }}
                                                                        </th>
                                                                        <th class="table-th text-center">
                                                                            {{ $clientes->name }}
                                                                            {{ $clientes->last_name }}</th>
                                                                        ...
                                                                        <th class="table-th text-center">
                                                                            <button
                                                                                class="btn btn-success btnUpdateFin"
                                                                                data-sale-id="{{ $pendientes->sales->id }}"
                                                                                title="Update to On transit">Change to
                                                                                Received</button>
                                                                        </th>
                                                                    </tr>
                                                                    @php
                ...
                <script type="text/javascript">
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        for (var i = 0; i < btnsUpdateFin.length; i++) {
                            btnsUpdateFin[i].addEventListener('click', function() {
                                var saleId = this.getAttribute('data-sale-id');
                                document.getElementById('IdFirma').value = saleId;
                                // Abre el modal para ingresar la firma
                                ...
                                document.getElementById('btnConfirmarFirma').addEventListener('click',
                                    function() {
                                        ...
                                        if (signaturePad.isEmpty()) {
                                            console.log(
                                                'El lienzo está vacío. Por favor, firme antes de continuar.');
                                        } else {
                                            var firma = signaturePad.toDataURL(); // Obtén la firma como base64
                                            console.log(firma); // Muestra la firma en la consola
                                            // Puedes enviarla a tu servidor o usarla como prefieras
                                        }
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                                    ...
                                                    @foreach ($operario as $operarios)
                                                        @if ($pendientes->id_transport == $operarios->id)
                                                            @foreach ($cliente as $clientes)
                                                                @if ($pendientes->sales->CustomerID == $clientes->id)
                                                                    <tr class="record">
                                                                        <td><button class="btn btn-warning btn-rounded"
                                                                                onclick="getSaleDetails({{ $pendientes->sales->id }})"><i
                                                                                    class="fas fa-list"></i></button>
                                                                        </td>
                                                                        <th class="table-th text-center">
                                                                            {{ $pendientes->sales->id }}</th>
                                                                        <th class="table-th text-center">
                                                                            {{ $pendientes->sales->fecha_firma }}</th>
                                                                        <th class="table-th text-center d-none">
                                                                            {{ $pendientes->sales->woocommerce_order_id }}
                                                                        </th>
                                                                        <th class="table-th text-center">
                                                                            {{ $clientes->name }}
                                                                            {{ $clientes->last_name }}</th>
                                    ...
                                    <div id="buttonsContainer">
                                        @if ($counter > 2)
                                            <button id="verMas" class="button" onclick="verMasRegistros()">Ver
                                                mas</button>
                                ...
                                <div id="total-orders" style="min-height: 295px;">
                                    <div id="apexchartsy7e34bu7h" class="apexcharts-canvas apexchartsy7e34bu7h light"
                                        style="width: 410px; height: 295px;"><svg id="SvgjsSvg1949" width="410"
                                            ...
                                            <g id="SvgjsG1951" class="apexcharts-inner apexcharts-graphical"
                                                ...
                                                <defs id="SvgjsDefs1950">
                                                    <clipPath id="gridRectMasky7e34bu7h">
                                                        <rect id="SvgjsRect1955" width="412" height="172" x="-1"
                                                            y="-1" rx="0" ry="0" fill="#ffffff"
                                                            opacity="1" stroke-width="0" stroke="none"
                                                            stroke-dasharray="0"></rect>
                                                    </clipPath>
                                                    <clipPath id="gridRectMarkerMasky7e34bu7h">
                                                        <rect id="SvgjsRect1956" width="412" height="172" x="-1"
                                                            y="-1" rx="0" ry="0" fill="#ffffff"
                                                            opacity="1" stroke-width="0" stroke="none"
                                                            stroke-dasharray="0"></rect>
                                                    </clipPath>
                                                    <linearGradient id="SvgjsLinearGradient1962" x1="0"
                                                        y1="0" x2="0" y2="1">
                                                ...
                                                <g id="SvgjsG1958"
                                                    ...
                                                    <g id="SvgjsG1959" class="apexcharts-series" seriesName="Sales"
                                                        ...
                                                        <path id="apexcharts-area-0"
                                                            ...
                                                            index="0" clip-path="url(#gridRectMasky7e34bu7h)"
                                                        ...
                                                        <g id="SvgjsG1960" class="apexcharts-series-markers-wrap">
                                                            <g class="apexcharts-series-markers">
                                                                <circle id="SvgjsCircle1980" r="0" cx="0"
                                                                    cy="0"
                                                ...
                                                <line id="SvgjsLine1975" x1="0" y1="0"
                                                    x2="410" y2="0" stroke="#b6b6b6"
                                                    stroke-dasharray="0" stroke-width="1"
                                                    class="apexcharts-ycrosshairs"></line>
                                                <line id="SvgjsLine1976" x1="0" y1="0"
                                                    x2="410" y2="0" stroke-dasharray="0"
                                            ...
                                            <rect id="SvgjsRect1953" width="0" height="0" x="0" y="0"
